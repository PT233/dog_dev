...
### 当前阻塞
- 2026-04-24：任务 7.3-7.6 阻塞（硬件接线未完成）
	- **原因**：树莓派硬件接线尚未完成，无法进行端到端联调测试
	- **影响**：任务 7.3（静止物体追踪）、7.4（缓慢移动物体）、7.5（遮挡恢复）、7.6（调参记录）均无法执行
	- **前置要求**：
		1. STM32 微控制器接线完成
		2. 舵机（SG90×4）接线完成
		3. MPU6050 IMU 接线完成（可选）
		4. 树莓派 UART 与 STM32 连接验证
		5. 树莓派相机与 USB 接线验证
	- **解决方案**：详见根目录 SETUP_GUIDE.md 和 hardware_wiring.html
