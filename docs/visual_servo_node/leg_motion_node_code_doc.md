...
`leg_motion_node` 的职责是把上游 `behavior_node` 输出的像素误差 `/behavior_node/output/pixel_error` 转换成四足机器人四路腿部舵机的目标角命令 `/leg_motion_node/output/servo_command`。
...
- 订阅 `/behavior_node/output/pixel_error`，把目标偏差转成控制量
- 订阅 `/uart_bridge_node/output/servo_state`，接收执行侧回传的舵机状态
- 以固定频率运行控制器和步态发生器
- 发布 `/leg_motion_node/output/servo_command` 给下游 `uart_bridge_node`
...
| 标准消息 | `geometry_msgs` | 订阅 `/behavior_node/output/pixel_error` 时使用 `Vector3` |
| 标准消息 | `sensor_msgs` | 发布 `/leg_motion_node/output/servo_command`、订阅 `/uart_bridge_node/output/servo_state` 时使用 `JointState` |
...
| 来源 | 类型 | 用途 |
| --- | --- | --- |
| `/behavior_node/output/pixel_error` | `geometry_msgs/msg/Vector3` | 视觉误差输入 |
| `/uart_bridge_node/output/servo_state` | `sensor_msgs/msg/JointState` | 舵机反馈输入 |
...
| --- | --- | --- | --- | --- |
| `/behavior_node/output/pixel_error` | `geometry_msgs/msg/Vector3` | `rclcpp::QoS(5)`，默认 `Reliable + Volatile + KeepLast(5)` | `pixel_error_callback()` | 接收视觉误差，更新当前目标偏差和最新时间戳 |
| `/uart_bridge_node/output/servo_state` | `sensor_msgs/msg/JointState` | `rclcpp::QoS(10)`，默认 `Reliable + Volatile + KeepLast(10)` | `servo_state_callback()` | 接收舵机反馈，缓存四条腿当前角度 |

### 3.2 发布的话题

| 话题名称 | 消息类型 | QoS | 发布频率 | 用途 |
| --- | --- | --- | --- | --- |
| `/leg_motion_node/output/servo_command` | `sensor_msgs/msg/JointState` | `rclcpp::QoS(5)`，默认 `Reliable + Volatile + KeepLast(5)` | 由定时器驱动，默认约 `30 Hz` | 发布四条腿的目标角命令，供 `uart_bridge_node` 编码发送给 STM32 |
...
#### 3.6.1 `/behavior_node/output/pixel_error`
...
#### 3.6.2 `/leg_motion_node/output/servo_command` 与 `/uart_bridge_node/output/servo_state`

两者都是 `sensor_msgs/msg/JointState`，但语义不同：

| 字段名 | 类型 | 在 `/leg_motion_node/output/servo_command` 中的含义 | 在 `/uart_bridge_node/output/servo_state` 中的含义 |
| --- | --- | --- | --- |
...
| --- | --- | --- | --- | --- | --- |
| `controller.control_rate_hz` | `float` | `30.0` | 建议 `> 0`；源码未校验 | 控制定时器频率 | 否 |
| `controller.deadband_pixels` | `float` | `5.0` | 建议 `>= 0` | 像素死区；误差绝对值小于该值时视为 0 | 否 |
| `servo.min_angle_deg` | `float` | `10.0` | 建议小于 `servo.max_angle_deg` | 舵机最小角限制，单位度 | 否 |
| `servo.max_angle_deg` | `float` | `170.0` | 建议大于 `servo.min_angle_deg` | 舵机最大角限制，单位度 | 否 |
| `servo.neutral_angle_deg` | `float` | `90.0` | 通常在机械中位附近 | 中立姿态角，单位度 | 否 |
| `gait.stride_amplitude_max_deg` | `float` | `20.0` | 建议 `>= 0` | 前进/后退步幅的最大摆动幅度，单位度 | 否 |
| `gait.turn_bias_max_deg` | `float` | `12.0` | 建议 `>= 0` | 左右转向附加偏置的最大幅度，单位度 | 否 |
| `gait.frequency_hz` | `float` | `2.0` | 建议 `> 0` | 步态相位推进频率 | 否 |
| `target.timeout_sec` | `float` | `0.5` | 建议 `>= 0` | 超过该时间未收到新误差时，认为目标丢失 | 否 |

### 4.2 转向 PID 参数

| 参数名 | 类型 | 默认值 | 取值范围 | 含义 | 是否动态可调 |
| --- | --- | --- | --- | --- | --- |
| `turn_controller.proportional_gain` | `float` | `0.05` | 一般 `>= 0` | 转向比例增益 | 否 |
| `turn_controller.integral_gain` | `float` | `0.001` | 一般 `>= 0` | 转向积分增益 | 否 |
| `turn_controller.derivative_gain` | `float` | `0.02` | 一般 `>= 0` | 转向微分增益 | 否 |

### 4.3 前进 PID 参数

| 参数名 | 类型 | 默认值 | 取值范围 | 含义 | 是否动态可调 |
| --- | --- | --- | --- | --- | --- |
| `forward_controller.proportional_gain` | `float` | `0.04` | 一般 `>= 0` | 前进比例增益 | 否 |
| `forward_controller.integral_gain` | `float` | `0.001` | 一般 `>= 0` | 前进积分增益 | 否 |
| `forward_controller.derivative_gain` | `float` | `0.02` | 一般 `>= 0` | 前进微分增益 | 否 |
...
class LegMotionControllerNode : public rclcpp::Node {
 ...
 private:
  ...
  bool has_pixel_error_ = false;   // 是否收到过误差输入
...
| 输入缓存 | `pixel_error_x_`、`pixel_error_y_`、`has_pixel_error_`、`last_pixel_error_time_` | 保存最新视觉误差与新鲜度 |
| 控制器状态 | `forward_controller_`、`turn_controller_`、`last_control_time_`、`gait_phase_rad_` | PID 状态和步态相位 |
...
- `turn_controller_`：把水平误差转换成左右转向偏置
- `forward_controller_`：把垂直误差转换成前进/后退步幅命令
...
3. 读取参数到成员变量
4. 构造两个 PID 控制器
5. 创建 `/behavior_node/output/pixel_error` 订阅
6. 创建 `/uart_bridge_node/output/servo_state` 订阅
7. 创建 `/leg_motion_node/output/servo_command` 发布器
...
```cpp
this->declare_parameter<float>("controller.control_rate_hz", 30.0f); // 控制频率
this->declare_parameter<float>("controller.deadband_pixels", 5.0f);      // 像素死区

control_rate_hz_ = this->get_parameter("controller.control_rate_hz").as_double(); // 读取参数
deadband_pixels_ = this->get_parameter("controller.deadband_pixels").as_double();         // 后续 PID 会使用
...
forward_controller_ = std::make_unique<PidController>(
  forward_proportional_gain, forward_integral_gain, forward_derivative_gain, deadband_pixels_,
...
flowchart TD
    ...
    B --> C[创建 turn_controller_]
    C --> D[创建 forward_controller_]
    D --> E[创建 /behavior_node/output/pixel_error 订阅]
    E --> F[创建 /uart_bridge_node/output/servo_state 订阅]
    F --> G[创建 /leg_motion_node/output/servo_command 发布器]
...
| 回调函数 | 触发条件 | 主要输入 | 处理结果 |
| --- | --- | --- | --- |
| `pixel_error_callback()` | 收到 `/behavior_node/output/pixel_error` | `geometry_msgs/msg/Vector3` | 更新最新误差、标记已有目标、刷新时间戳 |
| `servo_state_callback()` | 收到 `/uart_bridge_node/output/servo_state` | `sensor_msgs/msg/JointState` | 根据舵机名称更新四腿反馈角缓存 |
| `control_timer_callback()` | 定时器触发，默认约 30 Hz | 当前缓存的误差、PID 状态、步态相位 | 计算新一帧目标角并发布 `/leg_motion_node/output/servo_command` |

#### 5.4.1 `pixel_error_callback()`

触发条件：

- 上游 `behavior_node` 发布一条新的 `/behavior_node/output/pixel_error`

处理步骤：

1. 读取 `msg->x` 到 `pixel_error_x_`
2. 读取 `msg->y` 到 `pixel_error_y_`
3. 将 `has_pixel_error_` 置为 `true`
...
has_pixel_error_ = true;       // 标记已有视觉目标输入
...
#### 5.4.2 `servo_state_callback()`

触发条件：

- 下游 `uart_bridge_node` 或 `mock_uart_bridge_node` 发布一条 `/uart_bridge_node/output/servo_state`
...
7. 组合出四条腿的目标角，并做角度限幅
8. 把角度从“度”转成“弧度”，打包成 `JointState` 发布 `/leg_motion_node/output/servo_command`

完整流程图：

```mermaid
flowchart TD
    A[定时器触发 control_timer_callback] --> B[计算 dt]
    B --> C{target_fresh?}
    C -- 是 --> D[turn_error = -pixel_error_x_]
    D --> E[forward_error = -pixel_error_y_]
    E --> F[turn_bias = turn_controller.Update]
    F --> G[stride_command = forward_controller.Update]
    C -- 否 --> H[reset 两个 PID]
    ...
    O --> P[角度限幅]
    P --> Q[度转弧度]
    Q --> R[发布 /leg_motion_node/output/servo_command]
```
...
if (target_fresh) {
  ...
  turn_bias = turn_controller_->update(turn_error, dt);
  stride_command = forward_controller_->update(forward_error, dt);
} else {
  turn_controller_->reset();      // 目标丢失后清空控制器状态
...
| 误差源 | 控制通道 | 作用 |
| --- | --- | --- |
| `pixel_error_x_` | `turn_controller_` | 决定左右转向偏置 |
| `pixel_error_y_` | `forward_controller_` | 决定前后步幅强度 |
...
内部控制计算大多使用“度”，但 ROS 输出给 `/leg_motion_node/output/servo_command` 时使用“弧度”：
...
- `/leg_motion_node/output/servo_command.position[]` 却是**弧度**
...
| `PidController` 有 `set_gains()` 等方法 | 但当前节点没有参数更新回调，所以运行时不会自动调用这些方法 |
| 目标丢失后仍会发布 `/leg_motion_node/output/servo_command` | 内容是中立姿态，而不是停止发布 |
...
| 订阅回调 | `pixel_error_callback()`、`servo_state_callback()` |
| 后台线程 | 无 |
| 显式任务队列 | 无 |
...
这个入口会拉起 `mock_uart_bridge_node`，模拟 `/uart_bridge_node/output/servo_state` 反馈。
...
ros2 topic hz /leg_motion_node/output/servo_command
...
| `Leg motion: target=is_locked ...` | 定时器控制循环认为目标仍然有效 |
...
| 工具 | 推荐用途 |
| --- | --- |
| `rqt_graph` | 查看 `/behavior_node/output/pixel_error -> leg_motion_node -> /leg_motion_node/output/servo_command` 以及 `/uart_bridge_node/output/servo_state -> leg_motion_node` 的连通关系 |
| `rqt_plot` | 观察 `/behavior_node/output/pixel_error/x`、`/behavior_node/output/pixel_error/y`，也可以画 `/leg_motion_node/output/servo_command/position[0]` 一类的数值变化 |
...
| 现象 | 可能原因 | 排查方法 | 处理建议 |
| --- | --- | --- | --- |
| `/leg_motion_node/output/servo_command` 没有输出 | 节点未启动或环境未正确 `source` | `ros2 node list`、`ros2 topic hz /leg_motion_node/output/servo_command` | 先确认 `leg_motion_node` 已存在 |
| `/leg_motion_node/output/servo_command` 有输出，但机器人不动 | `uart_bridge_node` 未握手成功，未转发命令 | 查看 Pi 侧日志 | 先排查 UART 握手和硬件链路 |
| 机器人左右转方向反了 | 对 `pixel_error_x` 的正负理解与机械方向不一致 | `ros2 topic echo /behavior_node/output/pixel_error` 对照实际动作 | 调整控制符号或舵机安装方向 |
| 目标一丢就“站住” | 这是当前设计行为 | 检查日志是否频繁出现 `target=lost` | 适当调大 `target.timeout_sec` 或提高上游稳定性 |
| 动作抖动 | `controller.deadband_pixels` 太小、PID 太敏感 | 看 `/behavior_node/output/pixel_error` 曲线是否在 0 附近高频摆动 | 增大死区或调小 `proportional_gain/derivative_gain` |
| 步子太小或太大 | `gait.stride_amplitude_max_deg` 或 `forward_controller.*` 不合适 | 看 `/leg_motion_node/output/servo_command` 幅值变化 | 调整步幅上限和前进 PID |
| 收到 `/uart_bridge_node/output/servo_state` 但控制没明显变化 | 当前实现没有把反馈角闭环用进控制律 | 阅读 `servo_state_callback()` 与 `control_timer_callback()` | 如需真闭环，需要继续扩展算法 |

### 8.5 一个容易忽略的实现细节

当前节点会把 `/uart_bridge_node/output/servo_state` 里的角度转成度并存入 `current_leg_angles_deg_`，但后续控制计算没有使用这个数组。
...
| `scripts/verify_tracking.sh` | 端到端验证脚本 | 检查 `/behavior_node/output/pixel_error` 和 `/leg_motion_node/output/servo_command` 频率、链路连通性 |
...
| 建议测试项 | 价值 |
| --- | --- |
| `control_timer_callback()` 数值单元测试 | 验证不同误差输入下四腿角命令是否符合预期 |
| 目标超时测试 | 验证超时后是否稳定回到中立姿态 |
| `/uart_bridge_node/output/servo_state` 解析测试 | 验证不同舵机名字顺序下映射是否正确 |
...
| 项目 | 状态 |
| --- | --- |
| 订阅 `/behavior_node/output/pixel_error` | 已实现 |
| 订阅 `/uart_bridge_node/output/servo_state` | 已实现 |
| 定时发布 `/leg_motion_node/output/servo_command` | 已实现 |
| 双 PID 通道（转向 + 前进） | 已实现 |
| 简化正弦步态发生器 | 已实现 |
| 角度限幅与单位转换 | 已实现 |
| 目标超时后回到中立姿态 | 已实现 |
...
| 自动化节点级测试 | 目前缺少直接验证 `control_timer_callback()` 输出数值的自动化测试 |

### 10.4 对后续维护者的建议

如果你准备继续扩展这个节点，优先级比较高的方向有三个：

1. 把 `/uart_bridge_node/output/servo_state` 真正接入控制闭环，而不仅仅是缓存
2. 给参数增加动态更新回调，让 PID 调参不必重启节点
3. 为 `control_timer_callback()` 增加可重复的数值测试，避免以后改控制律时悄悄跑偏
