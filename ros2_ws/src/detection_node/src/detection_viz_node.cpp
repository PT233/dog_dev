...
cv::Scalar color_for_detection(const robot_interfaces::msg::Detection2D & detection)
{
  // 有 track_id 时按轨迹着色，否则按 class_id 着色，便于观察 ID 是否稳定。
  const std::string key = detection.track_id.empty() ?
...
std::string build_label(const robot_interfaces::msg::Detection2D & detection)
{
  // 标签格式尽量短，避免遮挡图像主体：ID、类别、置信度。
  std::ostringstream oss;
  if (!detection.track_id.empty()) {
    oss << "ID:" << detection.track_id << " ";
  }
  oss << "C:" << detection.class_id << " ";
  oss.setf(std::ios::fixed);
  oss.precision(2);
  oss << detection.confidence;
  return oss.str();
}
...
: rclcpp::Node("detection_viz_node", options)
{
  ...
  image_sub_ = this->create_subscription<sensor_msgs::msg::Image>(
    "~/input/image", qos,
    std::bind(&DetectionVizNode::image_callback, this, std::placeholders::_1));

  detection_sub_ = this->create_subscription<robot_interfaces::msg::Detection2DArray>(
    "~/input/tracked_objects", rclcpp::QoS(5).reliable(),
    std::bind(&DetectionVizNode::detections_callback, this, std::placeholders::_1));

  // 发布叠加后的图像，供 rqt_image_view 或录包检查视觉链路。
  image_pub_ = this->create_publisher<sensor_msgs::msg::Image>(
    "~/debug/detection_overlay", qos);

  RCLCPP_INFO(this->get_logger(), "DetectionVizNode started");
  RCLCPP_INFO(this->get_logger(), "Subscribed to private image and tracked_objects inputs");
  RCLCPP_INFO(this->get_logger(), "Publishing tracked overlay to ~/debug/detection_overlay");
}

void DetectionVizNode::image_callback(const sensor_msgs::msg::Image::SharedPtr msg)
{
  if (!msg) {
    return;
  }
...
void DetectionVizNode::detections_callback(
  const robot_interfaces::msg::Detection2DArray::SharedPtr msg)
{
  ...
  auto image_msg = image_for_stamp(msg->header.stamp);
  if (!image_msg) {
  ...
  publish_overlay(image_msg, msg);
}

sensor_msgs::msg::Image::SharedPtr DetectionVizNode::image_for_stamp(
  const builtin_interfaces::msg::Time & stamp)
{
  ...
  const uint64_t target_ns = stamp_to_nanoseconds(stamp);
  for (auto it = image_buffer_.rbegin(); it != image_buffer_.rend(); ++it) {
    // 先从最新图像往回找精确时间戳匹配。
    if (stamp_to_nanoseconds((*it)->header.stamp) == target_ns) {
      return *it;
    }
  }
  ...
  for (auto it = image_buffer_.rbegin(); it != image_buffer_.rend(); ++it) {
    const uint64_t image_ns = stamp_to_nanoseconds((*it)->header.stamp);
...
void DetectionVizNode::publish_overlay(
  ...
  const robot_interfaces::msg::Detection2DArray::SharedPtr & detections_msg)
{
  ...
  for (const auto & detection : detections_msg->detections) {
    ...
    const int x1 =
      std::max(0, static_cast<int>(std::lround(detection.center_x - detection.width * 0.5f)));
    const int y1 =
      std::max(0, static_cast<int>(std::lround(detection.center_y - detection.height * 0.5f)));
    const int x2 = std::min(
        static_cast<int>(overlay_msg->width) - 1,
        static_cast<int>(std::lround(detection.center_x + detection.width * 0.5f)));
    const int y2 = std::min(
        static_cast<int>(overlay_msg->height) - 1,
        static_cast<int>(std::lround(detection.center_y + detection.height * 0.5f)));

    if (x2 <= x1 || y2 <= y1) {
      continue;
    }

    const cv::Scalar color = color_for_detection(detection);
    cv::rectangle(image, cv::Point(x1, y1), cv::Point(x2, y2), color, 2);

    const std::string label = build_label(detection);
    int baseline = 0;
