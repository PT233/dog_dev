#ifndef UART_BRIDGE__UART_PROTOCOL_H_
#define UART_BRIDGE__UART_PROTOCOL_H_
...
typedef enum
{
  kUartCmdServoControl   = 0x01,    // 树莓派 → STM32：设置舵机目标角度
  kUartCmdQuery          = 0x02,    // 树莓派 → STM32：查询状态
  kUartCmdInitHandshake  = 0x10,    // 树莓派 → STM32：建立连接握手
  kUartCmdServoState     = 0x81,    // STM32 → 树莓派：舵机状态 v1（无时间戳）
  kUartCmdServoStateV2   = 0x82,    // STM32 → 树莓派：舵机状态 v2（含时间戳+序列号）
  kUartCmdSystemState    = 0x83,    // STM32 → 树莓派：系统状态（启动阶段上报）
  kUartCmdEmergencyStop  = 0xFF     // 双向：紧急停止
} UartCmdId;

// STM32 系统状态枚举（在 SYSTEM_STATE 帧中上报）
typedef enum
{
  kUartSystemStateBootCentering     = 0x01,    // 正在初始化归中
  kUartSystemStateWaitingConnection = 0x02,    // 等待握手
  kUartSystemStateActive            = 0x03,    // 就绪，可接受舵机指令
  kUartSystemStateError             = 0x7F     // 故障（IWDG 超时后重启）
} UartSystemState;
...
#endif  // UART_BRIDGE__UART_PROTOCOL_H_
