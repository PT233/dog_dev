...
std::vector<uint8_t> FrameEncoder::encode_servo_control(
  const ServoCmdItem * items,
  size_t count)
{
  ...
  return build_frame(kUartCmdServoControl, payload, payload_len);
}

std::vector<uint8_t> FrameEncoder::encode_single_servo(
  uint8_t servo_id,
  int16_t angle_x10,
  uint16_t duration_ms)
{
  ...
  return encode_servo_control(&item, 1);
}

std::vector<uint8_t> FrameEncoder::encode_init_handshake()
{
  ...
  return build_frame(kUartCmdInitHandshake,
                    reinterpret_cast<const uint8_t *>(&payload),
                    sizeof(payload));
}

std::vector<uint8_t> FrameEncoder::build_frame(
  uint8_t cmd_id,
  ...
  size_t payload_len)
{
  ...
  // 与 STM32 端 FrameParser::calculate_frame_crc() 计算范围一致
